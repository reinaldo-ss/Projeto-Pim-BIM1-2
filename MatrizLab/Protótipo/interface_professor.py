...
def validar_turma(codigoTurma):
    dadosTurma = utils.carregar_dados(utils.caminho_turmas)
...
def verMeusDados(professor):
    ...
    print(f"Sobrenome: {professor['sobrenome']}")
...
def criarMaterial(professor):
    ...
    item_dados = {}
    material = utils.carregar_dados(utils.caminho_materiais)
    todas_as_turmas = utils.carregar_dados(utils.caminho_turmas)
    ...
    item_dados["nomeProfessor"] = professor["nome"]

    material.append(item_dados)
    utils.salvar_dados(utils.caminho_materiais, material)
...
def verMateriais(professor):
    ...
    todos_os_materiais = utils.carregar_dados(utils.caminho_materiais)
    encontrou_material = False # 1. A Flag começa como Falsa.
    i = 1

    utils.limpaTela()
    print(f"====== Materiais Cadastrados por {professor['nome']} ======")
...
def deletarMaterial(professor):
    ...
    if confirmacao.upper() == "S":
        ...
        utils.salvar_dados(utils.caminho_materiais, materiais_atualizados)
        print("\n======================================================\n")
...
def alterarMaterial(professor):
    ...
    utils.salvar_dados(utils.caminho_materiais, todos_os_materiais)
    print("\n======================================================\n")
...
def criarAtividade(professor):
    ...
    atividades = utils.carregar_dados(utils.caminho_atividades)
    todas_as_turmas = utils.carregar_dados(utils.caminho_turmas)
    item_dados = {}
    ...
    utils.salvar_dados(utils.caminho_atividades, atividades)
...
def verAtividade(professor):
    ...
    # Usando uma variável de controle (flag).
    todas_as_atividades = utils.carregar_dados(utils.caminho_atividades)
    encontrou_atividade = False # 1. A Flag começa como Falsa.
    i = 1

    utils.limpaTela()
    print(f"====== Atividades Cadastradas por {professor['nome']} ======")
...
def deletarAtividade(professor):
    ...
    if confirmacao.upper() == "S":
        ...
        utils.salvar_dados(utils.caminho_atividades, atividades_atualizadas)
        print("\n======================================================\n")
...
def alterarAtividade(professor):
    ...
    # a alteração já está refletida. Agora só precisamos salvar a lista inteira.
    utils.salvar_dados(utils.caminho_atividades, todas_as_atividades)
    print("\n======================================================\n")
    input(f"Campo '{chave_selecionada}' atualizado com sucesso!\n\nPressione enter para voltar ao menu anterior...")
    utils.limpaTela()
...
def alterarNotaAluno():
    ...
    todas_as_turmas = utils.carregar_dados(utils.caminho_turmas)
    todos_os_alunos = utils.carregar_dados(utils.caminho_alunos)
    ...
    for i, aluno in enumerate(alunos_da_turma, start=1):
        ...
        print(f"  {i}. {aluno.get('nome')} (RA: {aluno.get('RA')}) - Nota Atual: {aluno.get('nota', 'N/A')}")
    ...
    nova_nota_str = None
    while True:
        nota_digitada = input(f"Digite a nova nota para {aluno_selecionado.get('nome')} (de 0 a 10): ").strip()
    ...
    utils.salvar_dados(utils.caminho_alunos, todos_os_alunos)
    print("\n======================================================\n")
    print(f"Sucesso! A nota de {aluno_selecionado.get('nome')} foi atualizada para {aluno_selecionado['nota']}.")
    input("Pressione enter para voltar ao menu anterior...")
    utils.limpaTela()
...
def iniciar_menu():
    ...
    while True:
        ...
        if professor_logado is None:
            utils.limpaTela()
            print("==== Sistema Login Professores ====\n")
        ...
        # Se alguém estiver logado, mostra o menu do usuario
            ...
            print(f"====== Bem vindo(a), {professor_logado['nome']}! (Conectado) ======\n")
