...
def cadastrar_novo_aluno():
    ...
    prosseguir = input("Você selecionou a opção 'cadastrar novo aluno', deseja continuar? (s/n)").strip()
    if prosseguir == "n":
        utils.voltar_menu()
        return alunos
    
    # Perguntas sobre os dados a serem inseridos, e suas devidas validações
    nome = validacao_geral("\nDigite o nome do Aluno:", validar_nome)
...
def modificar_alunos():
    ...
    print ("===== Editar Alunos =====")
    prosseguir = input("Você selecionou a opção 'editar alunos', deseja continuar? (s/n)").strip()
    if prosseguir == "n":
        utils.voltar_menu()
        return alunos
    ...
    ra_selecionado = input("\nDigite o RA desejado no campo a seguir: ")
    aluno_selecionado = None

    while aluno_selecionado == None:
        for aluno in alunos:
        ...
        if aluno_selecionado == None:
            utils.limpaTela()
            print("ERRO: Não foi encontrado um aluno com o RA digitado, tente novamente")
    ...
    while True:
        ...
        utils.limpaTela()
        print(f"'{chave_selecionada}' de '{aluno_selecionado['nome']} {aluno_selecionado['sobrenome']}' atualizado com sucesso!")
        utils.voltar_menu()
...
def apagar_alunos():
    ...
    utils.limpaTela()
    print("===== Apagar Alunos =====")
    prosseguir = input("Você selecionou a opção 'apagar aluno', deseja continuar? (s/n)").strip()
    if prosseguir == "n":
        utils.voltar_menu()
        return alunos
    ...
    raAluno = input("\nDigite o RA desejado no campo a seguir: ")
    confirmacao = input("Tem certeza que deseja apagar o aluno selecionado?(s/n): ").strip().lower()
...
def cadastrar_novo_professor():
    ...
    prosseguir = input("Você selecionou a opção 'cadastrar novo professor', deseja continuar? (s/n)").strip()
    if prosseguir == "n":
        utils.voltar_menu()
        return professores
    
    nome = validacao_geral("\nDigite o nome do Professor:", validar_nome)
    ...
    print(f"\nProfessor(a) {nome} {sobrenome} (RP: {rp_final}) cadastrado(a) com sucesso. \n")
    utils.voltar_menu()
    return professores
...
def modificar_professores():
    ...
    print("===== Modificar Professor =====")
    prosseguir = input("Você selecionou a opção 'modificar professor', deseja continuar? (s/n)").strip().lower()
    if prosseguir == "n":
        utils.voltar_menu()
        return professores
    ...
    rp_selecionado = input("\nDigite o RP desejado no campo a seguir: ")
    ...
    print(", ".join(professor_selecionado.keys())) # Mostra todas as chaves disponíveis
    print()
    print("=" * 42)
    ...
    while True:
        ...
        if chave_selecionada not in professor_selecionado: # Se chave digitada não existir
            ...
            utils.limpaTela()
            print(f"'{chave_selecionada}' de '{professor_selecionado['nome']} {professor_selecionado['sobrenome']}' atualizado com sucesso!")
            utils.voltar_menu()
...
def apagar_professores():
    ...
    prosseguir = input("Você selecionou a opção 'apagar professor', deseja continuar? (s/n)").strip()
    if prosseguir == "n":
        utils.voltar_menu()
        return professores
    ...
    for i, professor in enumerate(professores, start=1):
         print(f"{i}. {professor['nome']} {professor['sobrenome']} | RP: {professor['RP']}")
    rpProfessor = input("\nDigite o RP desejado no campo a seguir: ")
    confirmacao = input("\nTem certeza que deseja apagar o professor selecionado?(s/n): ")
